> **Original documentation** of the `jev-model-router` mod by Daniel (San) Ávila, from [claude-code-templates](https://github.com/davila7/claude-code-templates/tree/main/cli-tool/components/mods/productivity/jev-model-router), kept for reference. In jev-pilot this module runs inside one plugin (`jev-pilot`), so its options go under `pluginConfigs["jev-pilot"]`, its commands are `/jev-pilot:…`, and it is installed as described in the [README](../README.md), not with `--mod`. Some defaults differ; see the README.
